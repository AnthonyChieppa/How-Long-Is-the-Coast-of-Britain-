%% Task # 4 - Find the boundary, find the polynomial approximating the boundary
...
indicator = zeros(length(x), length(y)); % Prellocated array of length x and y
m_matrix = false(length(x), length(y)); % Prellocated matrix for speed

for i = 1:length(x) % Loop over all x values
    for j = 1:length(y) % Loop over all y values
